	...
	public function process(){
		...
		switch ( $this->type ){
            ...
            case 'getDeepLinkData':
                   $this->logger->debug('###Deep Link Data');
...
// 	         $default->initArgs( $type, $ref_id , $temp_id );
 	...
 	*/
 	...
 	private function getAllTags(){
		...
		$mobile_push_loyalty_tags = ModuleWiseTagsProvider::getTags( SupportedTagList::$MOBILEPUSH_LOYALTY);
		$ebill_tags =  ModuleWiseTagsProvider::getTags( SupportedTagList::$EBILL);
		$outbound_tags = ModuleWiseTagsProvider::getTags( SupportedTagList::$WECHAT_OUTBOUND);

		$tags = array('POINTSENGINE'=>$points_engine_tags, 'REFERRAL' => $referral_tags, 'ORG'=>$campaign_email_tags,'WECHAT'=>$we_chat_tags,'DVS'=>$dvs_email_tags, 'MOBILEPUSH' => $mobile_push_tags, 'MOBILEPUSH_LOYALTY'=>$mobile_push_loyalty_tags, 'OUTBOUND'=>$outbound_tags);
		$C_config_mgr = new ConfigManager();
    	$is_ebill_enabled = $C_config_mgr->getKey( "CONF_CAMPAIGN_EBILL_TEMPLATES_ENABLED" );
		if($is_ebill_enabled){
 	...
 	private function saveMobilePushTemplate(){
 		...
 		$scope = $params['scope'];
 		$mobpushScope = $params['mobpushScope'];
 		if(isset($params['is_favourite']) && $params['is_favourite']=="true"){
 			$is_favourite = 1;
 		} else{
 			$is_favourite = 0;
 		...
 		$C_template = $this->C_assets->processTemplate( $template_name , $file_path , $org_id , $uploaded_by , $asset_type, $file_content, $ref_id , $template_id, $tag, $mobpushScope, $is_favourite, $drag_drop_id );
 		$this->data['template_id'] = $C_template->getId();
	 		if($template_id && $template_id != -1){
	 			$this->data['success'] = _campaign('Template').' "'.$template_name.'" '._campaign('updated');
	 		} else {
 	...
 	private function getAllMobilePushTemplates($accountId = false){
		...
		$mobpushScope = $_REQUEST['mobpushScope'];

 		$this->C_assets = new CreativeAssetsManager();

 		if(isset($mobpushScope)){
 			$mobile_push_templates = $this->C_assets->getAllTemplates($this->org_id,$_REQUEST['scope'],$mobpushScope, 'GENERAL', $accountId);
 		}else{
 			$mobile_push_templates = $this->C_assets->getAllTemplatesCreativeAssets($this->org_id,$_REQUEST['scope'],'PUSH', 'GENERAL', $accountId);
 		}
 	...
 	private function getDeepLinkData(){
 		$this->logger->debug("sikri check".print_r($_REQUEST,true));
 		$channel_id = $_GET['channel_id'];
 		$account_id = $_GET['account_id'];

 		include_once 'business_controller/ChannelController.php';
		$channelController = new ChannelController();
		$configKeyData =  $channelController->getConfigKeyByChannelId($channel_id, $account_id);
		$deepLink;
		foreach ($configKeyData as $key => $value) {
