...
public static function getTags( SupportedTagList $type , array $tags = array() ){
	...
	switch ( $type ){
		...
		case SupportedTagList::$MOBILEPUSH_LOYALTY:
		return self::getMobilePushLoyaltyTags();
...
private function getWeChatOutboundTags(){
	...
	$tags = array(
			...
			_campaign('Customer Mobile') => '{{customer_mobile}}',
			_campaign('Customer Email') => '{{customer_email}}',
			_campaign('Loyalty Points') => '{{loyalty_points}}',
			_campaign('Promotion Points') => '{{promotion_points}}',
			_campaign('Slab Name') => '{{slab_name}}',
			_campaign('Store Name')	=>	'{{store_name}}',
			_campaign('Store Mobile') => '{{store_number}}',
			_campaign('Store Email') => '{{store_email}}',
			_campaign('User Id') => '{{user_id}}',
			_campaign('Voucher') => '{{voucher}}',
			_campaign('Voucher Validity') => '{{valid_till_date.FORMAT_3}}',
			_campaign('Refferal Unique Url') => '{{referral_unique_url}}',
			_campaign('Referral Unique Code') => '{{referral_unique_code}}',
			_campaign('Survey Url') => '{{SURVEY__URL__}}',
			_campaign('N/A') => '-'

		);

	//registration custom fields
	$custom_fields = $custom_field->getCustomFieldAttrsAsOption($currentorg->org_id, 'loyalty_registration');
	foreach ( $custom_fields as $k => $v ){
		$registration_tags[$k] = '{{custom_field.'.$k.'}}';
	}
	if($registration_tags)
		$tags = array_merge($tags,array('Registration custom fields' => $registration_tags));

	// $logger->debug("Tags sikri: ".print_r($tags,true));
	return $tags;
}
...
private function getWeChatTags(){
	...
	$tags = array(
			...
			_campaign('Redeemed Points') => '{{points_redeemed}}',
	...
	$custom_fields = $custom_field->getCustomFieldAttrsAsOption($currentorg->org_id, 'loyalty_registration');
	foreach ( $custom_fields as $k => $v ){
		$registration_tags[$k] = '{{custom_field_cu__'.$k.'}}';
	}
	if($registration_tags)
		$tags = array_merge($tags,array('Registration custom fields' => $registration_tags));
	...
	foreach ( $custom_fields as $k => $v ){
		$transaction_tags[$k] = '{{custom_field_tr__'.$k.'}}';
	}
	if($transaction_tags)
		$tags = array_merge($tags,array('Transaction custom fields' => $transaction_tags));

	//store custom fields
	$custom_fields = $custom_field->getCustomFieldAttrsAsOption($currentorg->org_id, 'store_custom_fields');
	foreach ( $custom_fields as $k => $v ){
		$store_tags[$k] = '{{custom_field_st__'.$k.'}}';
	}
	if($store_tags)
...
private function getMobilePushLoyaltyTags(){
	global $currentorg;
	...
	$customer_custom_field_tags = array();

	if( $result ){
		foreach( $result as $key => $value ){
			$customer_custom_field_tags[$key] = '{{custom_field_cu__'.$key.'}}';
		}
	}
	unset($result);
	...
	$transaction_custom_field_tags = array();

	if( $result ){
		foreach( $result as $key => $value ){
			$transaction_custom_field_tags[$key] = '{{custom_field_tr__'.$key.'}}';
		}
	}
	unset($result);
	...
	$result = $customFieldClass->getCustomFields( $currentorg->org_id, 
		'query_hash', 'name', 
		'name', 'store_custom_fields' 
		);
	$store_custom_field_tags = array();

	if( $result ){
		foreach( $result as $key => $value ){
			$store_custom_field_tags[$key] = '{{custom_field_st__'.$key.'}}';
		}
	}
	unset($result);
	...
	$tags = array(
		_campaign('Customer') => array(
			...
			_campaign('Slab Name') => '{{slab_name}}',
			_campaign('Slab Expiry Date') => '{{slab_expiry_date}}',
			_campaign('Custom Fields') => $customer_custom_field_tags
			),
		_campaign('Store') => array(
			_campaign('Name') => '{{store_name}}',
			_campaign('Mobile') => '{{store_number_mobile}}',
			_campaign('Landline') => '{{store_number_landline}}',
			_campaign('Email') => '{{store_email}}',
			_campaign('Address') =>	'{{store_address}}',
			_campaign('Custom Fields') => $store_custom_field_tags
			),
		_campaign('Transaction') => array(
			_campaign('Transaction Date')	=>	'{{bill_date}}',
			_campaign('Transaction Gross Amount')	=>	'{{bill_gross_amount}}',
			_campaign('Transaction Discount')	=>	'{{bill_discount}}',
			_campaign('Transaction Number')	=>	'{{bill_number}}',
			_campaign('Custom Feilds') => $transaction_custom_field_tags
			),
		_campaign('Coupon') => array(
			_campaign('Voucher Code') => '{{voucher_code}}',
			_campaign('Voucher Expiry Date') => '{{voucher_expiry_date}}'
			),
		_campaign('Tracker') => array(
			_campaign('Tracked Value') => '{{tracked_value}}', 
			_campaign('Current Aggregate') => '{{current_aggregate}}'
			),
		_campaign('Points') => array(
			_campaign('Points on Event') => '{{points_on_event}}',
			_campaign('Points on Event (in)') => '{{points_on_event_currency}}',
			_campaign('Points on action') => '{{points_on_action}}'
			)
		);

	if( count( $customer_custom_field_tags ) < 1 )
		unset( $tags[_campaign('Customer')][_campaign('Custom Fields')] );

	if( count( $transaction_custom_field_tags ) < 1 )
		unset( $tags[_campaign('Transaction')][_campaign('Custom Fields')] );

	if( count( $store_custom_field_tags ) < 1 )
		unset( $tags[_campaign('Store')][_campaign('Custom Fields')] );
